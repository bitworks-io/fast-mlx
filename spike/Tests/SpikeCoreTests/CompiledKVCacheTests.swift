...
final class CompiledKVCacheTests: XCTestCase {
    ...
    private func makeUnequalMLAKV(
        _ value: Float, n: Int = 1, dtype: DType = .float16
    ) -> (MLXArray, MLXArray) {
        // MLA semantic state: latent [B=1, kvHeads=1, n, R=5] plus rotary [B=1, 1, n, D=3].
        let latent = MLXArray.full([1, 1, n, 5], values: MLXArray(value)).asType(dtype)
        let rotary = MLXArray.full([1, 1, n, 3], values: MLXArray(value * 10)).asType(dtype)
    ...
    func testUpdatePreservesUnequalLatentAndRotaryDimensionsOffsetAndMask() {
        let cache = CompiledKVCache(capacity: 6)

        let (latentPrefill, rotaryPrefill) = makeUnequalMLAKV(1, n: 2)
        let (latentA, rotaryA) = cache.update(keys: latentPrefill, values: rotaryPrefill)
        XCTAssertEqual(latentA.shape, [1, 1, 6, 5])
        XCTAssertEqual(rotaryA.shape, [1, 1, 6, 3])
        XCTAssertEqual(cache.offsetArr.item(Int32.self), 2)
        XCTAssertEqual(cache.offset, 2)

        let (latentDecode, rotaryDecode) = makeUnequalMLAKV(2, n: 1)
        let (latentB, rotaryB) = cache.update(keys: latentDecode, values: rotaryDecode)

        XCTAssertEqual(latentB.shape, [1, 1, 6, 5])
        XCTAssertEqual(rotaryB.shape, [1, 1, 6, 3])
        XCTAssertEqual(cache.offsetArr.item(Int32.self), 3)
        XCTAssertEqual(cache.offset, 3)
        XCTAssertEqual(latentB[0, 0, 1, 4].item(Float.self), 1)
        XCTAssertEqual(latentB[0, 0, 2, 4].item(Float.self), 2)
        XCTAssertEqual(latentB[0, 0, 3, 0].item(Float.self), 0)
        XCTAssertEqual(rotaryB[0, 0, 1, 2].item(Float.self), 10)
        XCTAssertEqual(rotaryB[0, 0, 2, 2].item(Float.self), 20)
        XCTAssertEqual(rotaryB[0, 0, 3, 0].item(Float.self), 0)
        ...
        let expected: [Bool] = [true, true, true, true, false, false]
    ...
    func testCaptureRejectsUnequalLatentAndRotaryDimensionsBeforeSerialization() {
        let cache = CompiledKVCache(capacity: 6)
        let (latent, rotary) = makeUnequalMLAKV(1, n: 2)
        _ = cache.update(keys: latent, values: rotary)

        assertSnapshotError(.unsupportedUnequalFeatureDimensions) {
            _ = try cache.captureSnapshot(logicalTokenCount: 2)
        }
        XCTAssertEqual(cache.offsetArr.item(Int32.self), 2)
        XCTAssertEqual(cache.offset, 2)
        XCTAssertEqual(cache.keysBuf?.shape, [1, 1, 6, 5])
        XCTAssertEqual(cache.valuesBuf?.shape, [1, 1, 6, 3])
    }
