...
// File-scope (not a class member) rather than `private`: called from every `XCTestCase` subclass
// in this target that needs a fixture backend configuration, including
// `OffloadedNGramPlanCheckTests.swift` (which moved out of this file but still shares this
// builder rather than duplicating it -- see that file's header comment) and the projection-only
// `OffloadedNGramPlanTwinContractTests.swift`. An instance method on `MLXScalarServingTests` would
// not be callable from those unrelated `XCTestCase` subclasses even at `internal` access, since
// Swift member lookup does not cross between sibling types.
func fixtureBackendConfiguration() -> ScalarServingBackendConfiguration {
