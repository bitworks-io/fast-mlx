...
final class MeasurementCorpusTests: XCTestCase {
    ...
    func testRealCheckedInCorpusLoadsAndHasLongContextEntry() throws {
        ...
        // Character-count proxy for token count (the real tokenizer check happens on a qualified
        // benchmark host where the model's tokenizer is available) — well over the 4x margin
        // needed to clear 4K tokens.
