...
final class SwiftEngineDriverConfigTests: XCTestCase {
    func testAbsorbedMLAHarnessAdmissionAllowsScalarFP16ForDeepSeekV3()
        throws
    {
        for config in [
            RunConfig(),
            RunConfig(kvQuant: "fp16"),
        ] {
            assertAbsorbedMLAHarnessAdmissionAccepts(
                modelType: "deepseek_v3",
                config: config)
        }
    }

    func testAbsorbedMLAHarnessAdmissionRejectsNonScalarKVForDeepSeekV3()
        throws
    {
        let admission = try makeAdmission(layerCount: 3)
        let selection = try makeSelection(admission: admission)

        for config in [
            RunConfig(kvQuant: "affine-k4v2-g64"),
            RunConfig(kvQuant: "tqB2"),
            RunConfig(kvQuant: "kvarn-k4v2-g128"),
            RunConfig(
                kvQuant: selection.cellID,
                kvtunerSelection: selection),
        ] {
            assertAbsorbedMLAHarnessAdmissionRejects(
                modelType: "deepseek_v3",
                config: config)
        }
    }

    func testAbsorbedMLAHarnessAdmissionRejectsCompressedAttentionForDeepSeekV3()
        throws
    {
        for request in [
            CompressedKVAttentionRequest.materialize,
    ...
    func testAbsorbedMLAHarnessAdmissionRejectsExactPrefixRequestForDeepSeekV3()
        throws
    {
        assertAbsorbedMLAHarnessAdmissionRejects(
            modelType: "deepseek_v3",
            config: RunConfig(
                exactPrefixRequest: try makeExactPrefixRequestContext()))
        XCTAssertThrowsError(
            try validateAbsorbedMLAHarnessAdmission(
                modelType: "deepseek_v3",
                config: RunConfig(),
                exactPrefixCachePolicyEnabled: true))
    }

    func testAbsorbedMLAHarnessAdmissionRejectsSpecDecodeForDeepSeekV3()
        throws
    {
        let speculativeConfigs = [
            RunConfig(specDecode: "pld"),
            RunConfig(specDecode: "dspark"),
            RunConfig(specNgram: 3),
            RunConfig(specMaxDraft: 8),
            RunConfig(specCompiledVerify: true),
        ]
        for config in speculativeConfigs {
            assertAbsorbedMLAHarnessAdmissionRejects(
                modelType: "deepseek_v3",
                config: config)
        }
    ...
    func testAbsorbedMLAHarnessAdmissionLeavesQwen3AndOtherSelectionsUnchanged()
        throws
    {
        let admission = try makeAdmission(layerCount: 3)
        let selection = try makeSelection(admission: admission)
        ...
        for modelType in ["qwen3", "llama"] {
            for config in existingSelections {
                assertAbsorbedMLAHarnessAdmissionAccepts(
                    modelType: modelType,
                    config: config)
            }
    ...
    private func assertAbsorbedMLAHarnessAdmissionAccepts(
        modelType: String,
        config: RunConfig,
    ...
    private func assertAbsorbedMLAHarnessAdmissionRejects(
        modelType: String,
        config: RunConfig,
        file: StaticString = #filePath,
        line: UInt = #line
    ...
    private func makeExactPrefixRequestContext() throws
        -> ExactPrefixRequestContext
    {
        try ExactPrefixRequestContext(
            isolationNamespaceSHA256: String(repeating: "1", count: 64),
            promptTemplateSHA256: String(repeating: "2", count: 64),
            toolsSHA256: String(repeating: "3", count: 64))
    }
