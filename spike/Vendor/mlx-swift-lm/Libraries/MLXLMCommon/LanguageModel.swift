...
/// Lets a model whose sanitizer derives new module paths inherit the source checkpoint's
/// per-layer quantization policy. Returning nil preserves the sanitized path unchanged.
public protocol WeightQuantizationPathResolver {
    func sourceWeightQuantizationPath(
        for sanitizedPath: String
    ) -> String?
}

/// Lets a model validate sanitizer-derived quantized tensors against the loader policy before
/// quantized modules are constructed or any parameters are updated.
public protocol SanitizedWeightQuantizationValidator {
    func validateSanitizedWeightQuantization(
