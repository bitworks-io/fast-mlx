...
///
/// This provides a single function that automatically routes to cache-owned, quantized, or
/// regular attention based on cache type, matching Python's `scaled_dot_product_attention`.

/// Automatic attention with cache update
///
/// This function matches Python's `scaled_dot_product_attention` in base.py:
/// - Gives a cache implementing `AttentionKVCacheProtocol` first refusal over update + attention
/// - Otherwise detects `QuantizedKVCacheProtocol` and routes to quantized attention
/// - Retains `MLXFast.scaledDotProductAttention` for every other cache
...
public func attentionWithCacheUpdate(
    ...
    if let attentionCache = cache as? AttentionKVCacheProtocol {
