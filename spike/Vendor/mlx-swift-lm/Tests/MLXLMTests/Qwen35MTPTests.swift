...
struct Qwen35MTPMetalTests {
    @Test
    func testStandaloneBareTensorNamesNormalizeIntoMTPModuleNamespace() throws {
        let cfg = try JSONDecoder().decode(
            MLXLLM.Qwen35Configuration.self,
            from: Data(qwen35StandaloneMTPConfigJSON().utf8))
        let drafter = MLXLLM.Qwen35MTPDraftModel(cfg, preconvertedNorms: true)
        ...
        let sanitized = drafter.sanitize(weights: [
            "fc.biases": MLXArray.zeros([16, 1]),
            "layers.0.input_layernorm.weight": MLXArray.zeros([16]),
            "norm.weight": MLXArray.zeros([16]),
            "pre_fc_norm_hidden.weight": MLXArray.zeros([16]),
        ])

        #expect(sanitized["fc.biases"] == nil)
        #expect(sanitized["mtp.fc.biases"] != nil)
        #expect(sanitized["mtp.layers.0.input_layernorm.weight"] != nil)
        #expect(sanitized["mtp.norm.weight"] != nil)
        #expect(sanitized["mtp.pre_fc_norm_hidden.weight"] != nil)
    }
    ...
    @Test
    func testQwen35TextScalarCallRemainsBitIdenticalWithoutMTPState() throws {
        MLXRandom.seed(44)
        let cfg = try JSONDecoder().decode(
            MLXLLM.Qwen35TextConfiguration.self,
            from: Data(qwen35TextConfigJSON(mtpLayers: 1).utf8))
        ...
        let scalar = model(tokens, cache: nil)
        let stateAware = model(LMInput.Text(tokens: tokens), cache: nil, state: nil)
        eval(scalar, stateAware.logits)

        #expect(stateAware.state == nil)
        #expect(allClose(scalar, stateAware.logits, rtol: 0, atol: 0).item(Bool.self))
    }
    ...
    @Test
    func testQwen35VLMTextOnlyMTPReprimeMatchesStateThreadedScalarReference() throws {
        MLXRandom.seed(45)
        let cfg = try JSONDecoder().decode(
            MLXVLM.Qwen35Configuration.self,
            from: Data(qwen35VLMConfigJSON(
        ...
        let referenceCache = model.newCache(parameters: parameters)
        let prepared = try model.prepare(input, cache: referenceCache, windowSize: nil)
        guard case .logits(let preparedOutput) = prepared else {
            Issue.record("Qwen35 VLM text-only prepare must return logits")
            return
        }
        let firstToken = sampler.sample(
            logits: preparedOutput.logits[0..., -1, 0...])
        var referenceState = preparedOutput.state ?? LMOutput.State()
        referenceState[mtpEmitFlagKey] = true
        let referencePrimed = model(
            LMInput.Text(tokens: firstToken), cache: referenceCache, state: referenceState)
        let secondToken = sampler.sample(
            logits: referencePrimed.logits[0..., -1, 0...])
        ...
        eval(secondToken, iterator.y.tokens)
        #expect(iterator.y.tokens.item(Int.self) == secondToken.item(Int.self))
        #expect(
            iterator.mainCache.map { $0.offset } == referenceCache.map { $0.offset },
            "MTP re-prime and scalar reference must commit the same prompt/token span")

        let referenceContinuation = model(
            LMInput.Text(tokens: secondToken),
            cache: referenceCache,
            state: referencePrimed.state)
        let mtpContinuation = model(
            iterator.y,
            cache: iterator.mainCache,
            state: iterator.mainState)
        eval(referenceContinuation.logits, mtpContinuation.logits)
        #expect(
            allClose(
                referenceContinuation.logits, mtpContinuation.logits,
                rtol: 1e-5, atol: 1e-5
            ).item(Bool.self),
            "text-only Qwen VLM continuation positions must match the state-threaded scalar path")
    }

    @Test
    func testQwen35VLMReprimeAlignsDrafterCacheBeforeAndAfterFirstCommit() throws {
        MLXRandom.seed(51)
        let cfg = try JSONDecoder().decode(
            MLXVLM.Qwen35Configuration.self,
            from: Data(qwen35VLMConfigJSON(
        ...
        let initialDrafterState = try #require(iterator.drafterState)
        let initialTargetOffset = try #require(
            iterator.mainState?[mtpSharedKVOffsetsKey]?["full_attention"])
        #expect(initialDrafterState.nextPosition == initialTargetOffset)
        #expect(initialDrafterState.cache.allSatisfy { $0.offset == initialTargetOffset })

        #expect(iterator.next() != nil, "committed re-prime token")
        #expect(iterator.next() != nil, "uncommitted bonus token")
        #expect(iterator.next() != nil, "first verified token")

        let committedDrafterState = try #require(iterator.drafterState)
        let committedTargetOffset = try #require(
            iterator.mainState?[mtpSharedKVOffsetsKey]?["full_attention"])
        #expect(committedDrafterState.nextPosition == committedTargetOffset)
        #expect(committedDrafterState.cache.allSatisfy { $0.offset == committedTargetOffset })
    }

    @Test
    func testQwen35VLMDiscardedReprimeTokenRestoresPromptOnlyHybridCache() throws {
        MLXRandom.seed(46)
        let cfg = try JSONDecoder().decode(
            MLXVLM.Qwen35Configuration.self,
            from: Data(qwen35VLMConfigJSON(
        ...
        let referenceCache = model.newCache(parameters: parameters)
        _ = try model.prepare(input, cache: referenceCache, windowSize: nil)
        ...
        #expect(iterator.mainCache.map { $0.offset } == referenceCache.map { $0.offset })
        for (actualCache, referenceCache) in zip(iterator.mainCache, referenceCache) {
            let actual = actualCache.state
            let reference = referenceCache.state
            #expect(actual.count == reference.count)
            for (actualArray, referenceArray) in zip(actual, reference) {
                eval(actualArray, referenceArray)
                #expect(actualArray.shape == referenceArray.shape)
                #expect(
                    allClose(actualArray, referenceArray, rtol: 2e-3, atol: 2e-3)
                        .item(Bool.self))
            }
        }
    }
    ...
    @Test
    func testQwen35VLMWrappedAttentionCacheFallsBackBeforeHybridReprime() throws {
        MLXRandom.seed(47)
        let cfg = try JSONDecoder().decode(
            MLXVLM.Qwen35Configuration.self,
            from: Data(qwen35VLMConfigJSON(
        ...
        let parameters = GenerateParameters(
            maxTokens: 4, maxKVSize: 8, temperature: 0)
        ...
        #expect(
            iterator.passthroughReason
                == "hybrid cache cannot checkpoint one-token MTP re-prime")
        let first = iterator.next()
        #expect(first != nil)
        iterator.discardGeneratedToken()
        iterator.finalizeGeneration()
        ...
        for (actualCache, referenceCache) in zip(iterator.mainCache, referenceCache) {
            ...
            for (actualArray, referenceArray) in zip(actual, reference) {
                ...
                #expect(actualArray.shape == referenceArray.shape)
                #expect(
                    allClose(actualArray, referenceArray, rtol: 0, atol: 0)
                        .item(Bool.self))
            }
        }
    }
    ...
    @Test
    func testQwen35VLMWrappedDenseAttentionCacheFallsBackBeforeReprime() throws {
        MLXRandom.seed(48)
        let cfg = try JSONDecoder().decode(
            MLXVLM.Qwen35Configuration.self,
            from: Data(qwen35VLMConfigJSON(
                mtpLayers: 1, hiddenLayers: 1, fullAttentionInterval: 1).utf8))
        let model = MLXVLM.Qwen35(cfg)
        let drafter = MLXVLM.Qwen35VLMNextNDraftModel(cfg)
        let input = LMInput(tokens: MLXArray([Int32(1), 2, 3, 4, 5, 6, 7, 8]))
        let parameters = GenerateParameters(
    ...
    @Test
    func testQwen35VLMHybridVerifyBoundaryFallsBackBeforeRotatingCacheWrap() throws {
        try assertQwen35VLMVerifyBoundaryFallsBackBeforeRotatingCacheWrap(
            seed: 49, hiddenLayers: 2, fullAttentionInterval: 2)
    }

    @Test
    func testQwen35VLMDenseVerifyBoundaryFallsBackBeforeRotatingCacheWrap() throws {
        try assertQwen35VLMVerifyBoundaryFallsBackBeforeRotatingCacheWrap(
            seed: 50, hiddenLayers: 1, fullAttentionInterval: 1)
    }
    ...
    func testQwen35GDNCheckpointMatchesPrefixWithoutReplayingProjections() throws {
        ...
        for (index, pair) in zip(restored, expected).enumerated() {
            let (actual, reference) = pair
            eval(actual, reference)
            #expect(actual.shape == reference.shape)
            if index == 0 {
                // The checkpoint reuses the two-token verification projection. MLX may
                // schedule the independent one-token reference matmul differently, so
                // compare its convolution window at fp32 accumulation tolerance.
                #expect(allClose(actual, reference, rtol: 2e-3, atol: 2e-3).item(Bool.self))
            } else {
                #expect(allClose(actual, reference, rtol: 1e-5, atol: 1e-5).item(Bool.self))
            }
        }

        let continuation = MLXRandom.normal([1, 1, 16])
        let restoredOutput = layer(continuation, cache: speculativeCache)
        let expectedOutput = layer(continuation, cache: prefixCache)
        eval(restoredOutput, expectedOutput)
        #expect(
            allClose(restoredOutput, expectedOutput, rtol: 1e-4, atol: 1e-4)
                .item(Bool.self))
    }
    ...
    func testQwen35VLMGDNCheckpointMatchesPrefix() throws {
        ...
        #expect(allClose(speculative, full, rtol: 1e-5, atol: 1e-5).item(Bool.self))
        #expect(speculativeCache.restoreSpeculativeCheckpoint())
        for (index, pair) in zip(speculativeCache.state, prefixCache.state).enumerated() {
            let (actual, reference) = pair
            eval(actual, reference)
            #expect(actual.shape == reference.shape)
            if index == 0 {
    ...
    @Test
    func testReprimeCheckpointFailsClosedWithoutPartiallyCheckpointingHybridCache() {
        let complete = MambaCache()
        complete[0] = MLXArray.ones([1, 1, 4])
        complete[1] = MLXArray.ones([1, 2, 2, 2])

        let incomplete = MambaCache()
        incomplete[0] = MLXArray.zeros([1, 1, 4])

        #expect(
            !checkpointSpeculativePromptCacheForOneTokenReprime([complete, incomplete]))
        #expect(!complete.hasSpeculativeCheckpoint)
        #expect(!incomplete.hasSpeculativeCheckpoint)
    }

    @Test
    func testQwen35VLMRejectsMultimodalSpeculativePrefillUntilStateIsPreserved() throws {
        let config = try JSONDecoder().decode(
            MLXVLM.Qwen35Configuration.self,
            from: Data(qwen35VLMConfigJSON(mtpLayers: 1).utf8))
        let drafter = MLXVLM.Qwen35VLMNextNDraftModel(config)
        let textOnly = LMInput(tokens: MLXArray([Int32(1), 2, 3]))
        let multimodal = LMInput(
            text: textOnly.text,
            image: .init(pixels: MLXArray.zeros([1, 1, 1, 3])))

        #expect(drafter.supportsSpeculation(for: textOnly))
        #expect(!drafter.supportsSpeculation(for: multimodal))
    }
}

private func assertQwen35VLMVerifyBoundaryFallsBackBeforeRotatingCacheWrap(
    seed: UInt64,
    hiddenLayers: Int,
    ...
    let cfg = try JSONDecoder().decode(
        ...
        from: Data(qwen35VLMConfigJSON(
            ...
            fullAttentionInterval: fullAttentionInterval).utf8))
    let model = MLXVLM.Qwen35(cfg)
    let drafter = MLXVLM.Qwen35VLMNextNDraftModel(cfg)
    ...
    let firstToken = sampler.sample(
        logits: preparedOutput.logits[0..., -1, 0...])
    let firstStep = model(
        LMInput.Text(tokens: firstToken),
        cache: referenceCache,
        state: preparedOutput.state)
    let secondToken = sampler.sample(logits: firstStep.logits[0..., -1, 0...])
    let secondStep = model(
        LMInput.Text(tokens: secondToken),
        cache: referenceCache,
        state: firstStep.state)
    let thirdToken = sampler.sample(logits: secondStep.logits[0..., -1, 0...])
    ...
    var iterator = try MTPSpeculativeTokenIterator(
        ...
        parameters: parameters,
        blockSize: 2)
    #expect(iterator.next() == firstToken.item(Int.self))
    #expect(iterator.next() == secondToken.item(Int.self))
    #expect(iterator.next() == thirdToken.item(Int.self))
    #expect(
        iterator.passthroughReason
            == "main cache cannot preserve rollback through MTP verify")
    ...
    for (actualCache, referenceCache) in zip(iterator.mainCache, referenceCache) {
        ...
        for (actualArray, referenceArray) in zip(actual, reference) {
            eval(actualArray, referenceArray)
            #expect(actualArray.shape == referenceArray.shape)
            guard actualArray.shape == referenceArray.shape else { continue }
            #expect(
                allClose(actualArray, referenceArray, rtol: 0, atol: 0)
                    .item(Bool.self))
        }
...
private func qwen35TextConfigJSON(
    ...
    // MLX's GDN Metal kernel assigns `linear_key_head_dim / 32` values per SIMD lane;
    // a narrower fixture produces an unsupported zero-work specialization before cache assertions.
    """
    {
