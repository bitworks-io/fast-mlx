...
/// Phase-1 absorbed MLA is intentionally narrower than the harness's general cache surface.
/// Keep this gate independent of MLX/model loading so every excluded route fails before it can
/// allocate or mutate cache state.
func validateAbsorbedMLAHarnessAdmission(
    modelType: String,
    config: RunConfig,
    exactPrefixCachePolicyEnabled: Bool = false
) throws {
    guard modelType == "deepseek_v3" else {
        return
    }
    guard config.kvQuant == nil || config.kvQuant == "fp16",
        config.kvtunerSelection == nil
    else {
        throw SwiftEngineDriverError.unsupportedConfig(
            "deepseek_v3 absorbed MLA requires scalar fp16/bfloat16 KV state")
    }
    guard config.compressedKVAttention == nil,
        config.compressedKVAttentionExpectedCheckpointContentSHA256 == nil
    else {
        throw SwiftEngineDriverError.unsupportedConfig(
            "deepseek_v3 absorbed MLA does not admit compressed-attention routes")
    }
    guard config.specDecode == nil,
        config.specNgram == nil,
        config.specMaxDraft == nil,
        config.specCompiledVerify == nil
    else {
        throw SwiftEngineDriverError.unsupportedConfig(
            "deepseek_v3 absorbed MLA does not admit speculative decoding")
    }
    guard config.exactPrefixRequest == nil,
        !exactPrefixCachePolicyEnabled
    else {
        throw SwiftEngineDriverError.unsupportedConfig(
            "deepseek_v3 absorbed MLA does not admit exact-prefix reuse")
    }
}
...
func loadSwiftDriver(
    ...
    struct ModelTypeConfiguration: Decodable {
        let modelType: String
    ...
    let modelDirectory = URL(fileURLWithPath: modelPath)
    let modelType = try JSONDecoder().decode(
        ModelTypeConfiguration.self,
        from: Data(
            contentsOf: modelDirectory.appendingPathComponent(
                "config.json")))
        .modelType
    try validateAbsorbedMLAHarnessAdmission(
        modelType: modelType,
