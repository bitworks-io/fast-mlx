...
public enum CompiledKVCacheSnapshotError: Error, Equatable, Sendable {
    ...
    case unsupportedUnequalFeatureDimensions
}
...
public final class CompiledKVCache: KVCache, Updatable, MLALatentKVCache {
    ...
    private static func validateLiveBuffers(
        ...
        guard keys.shape.count == 4,
            ...
            values.dim(2) == capacity,
            values.dim(3) > 0
        else {
            throw CompiledKVCacheSnapshotError.shapeMismatch
        }
        guard values.dim(3) == keys.dim(3) else {
            throw CompiledKVCacheSnapshotError
                .unsupportedUnequalFeatureDimensions
        }
