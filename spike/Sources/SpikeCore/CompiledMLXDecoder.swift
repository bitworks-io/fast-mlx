...
public enum CompiledMLXDecoderCacheAdmissionError: Error, Equatable, Sendable {
    case absorbedDeepseekV3RequiresFP16(requested: KVCacheKind)
}
...
public struct CompiledMLXDecoder: Decoder {
    ...
    static func validateKVCacheAdmission(
        kvCache: KVCacheKind,
        isAbsorbedDeepseekV3: Bool
    ) throws {
        guard isAbsorbedDeepseekV3 else { return }
        guard kvCache == .fp16 else {
            throw CompiledMLXDecoderCacheAdmissionError
                .absorbedDeepseekV3RequiresFP16(requested: kvCache)
        }
    }
    ...
    private mutating func prefillCore(_ promptTokens: [Int]) -> MLXArray {
        ...
        if caches.isEmpty {
            do {
                try Self.validateKVCacheAdmission(
                    kvCache: kvCacheKind,
                    isAbsorbedDeepseekV3: model is DeepseekV3Model)
            } catch {
                preconditionFailure(
