...
- **Captured:** 2026-07-09
- **Status:** backlog (planning-ready after 2026-07-12 source correction)
- **Task type:** engine capability / KV-memory optimization
- **Priority:** high — exact, model-specific, and now backed by two MLX implementation references
...
The famous MLA memory savings are a property of an **absorbed** cache implementation (cache only the compressed latent `kv_lora_rank 512 + qk_rope 64` per layer, re-project per query at attention time). Absorbed = **68.6 KiB/token → 2.14 GiB @32K — 71× smaller**, and it should be **exact/lossless** (an algebraic reformulation, not an approximation). The pinned Swift DeepSeek-V3 path still expands K/V before caching, so the fast-mlx gap is real.

> **Source correction, 2026-07-12:** the earlier claim that nobody had shipped absorbed MLA
> in MLX was false. Python [`mlx-lm` v0.30.6](https://github.com/ml-explore/mlx-lm/releases/tag/v0.30.6)
> shipped DSV3 MLA. The pinned `mlx-swift-lm` revision also contains an absorbed latent-cache
> analogue in `GLM4MOELite.swift`: it caches `kvLatent + kPe`, transforms the query through
> `embedQ`, transforms the output through `unembedOut`, and converts `kv_b_proj` weights at
> load. `DeepseekV3.swift` itself remains unabsorbed. Those two references make this task
> planning-ready with a much narrower design search; its own design review and implementation
...
- Port the algebra and weight conversion from current Python DSV3 / Swift GLM4-MoE-Lite;
  do not invent a third formulation.
- Keep latent-cache quantization out of phase 1. First prove exact absorption, then decide
  whether compressing an already-small latent is worth a separate dial point.

## Next planning step

Own design note + implementation plan (do not fold into the context-operability feature).
Use the upstream Python implementation and pinned Swift GLM path as the two code oracles.
Gate: near-float-noise teacher-forced logits/attention against the as-implemented decompressed
baseline, then memory and speed. Referenced from the operability spec §7 (backlog) and §8
(DeepSeek-R1 honesty case): `docs/superpowers/specs/2026-07-09-system-aware-context-operability.md`.
