...
**Dial axes (v1):** weight-quant preset × KV-quant tier (`fp16` / `8-bit` / `turbo4` Hadamard-rotated) × spec-decode toggle (PLD + DSpark) × chunked-prefill size. (2-bit KV / real TurboQuant / MTP are experimental flags, not dial tiers, until measured.)
...
- **Chunked prefill.** Prefill large prompts in bounded chunks (default 8192, configurable) for GPU memory pressure, cache-alignment awareness (chunk stride must stay aligned with SSM-checkpoint stride on hybrid models — misalignment silently kills prefix-cache hits), and a natural `Task.checkCancellation()` point between chunks.
...
**Owner additions to resolve during planning:**
- **Nemotron 3 Ultra** — NEW; not in the current ladder. The engine has *Nemotron-H* (a different, smaller hybrid arch). Confirm what Nemotron 3 Ultra is, MLX availability, the architecture support it needs, and 256/512GB fit.
- **Ornith** — already in the ladder at **Tier C4, flagged aggregator-sourced / unverified**. Verify it is real, its license (unknown), MLX availability, and fit.
- **Deliverable:** a memory-fit table per model on 256GB and 512GB, plus any net-new architecture support required (the ladder flags GLM / Phi / command-r / Kimi / DeepSeek-V4-Pro as needing archs the Zig engine lacked).
...
- **Catalog unknowns** — Nemotron 3 Ultra / Ornith (§9).
