...
- ✅ Target model **`Qwen3-30B-A3B-Instruct-2507-4bit`** present — actual weights at **`~/perf-work/models/Qwen3-30B-A3B-Instruct-2507-4bit/`** (flat dir; the `~/.cache/huggingface/hub` path is an empty ref, NOT the weights). Also on box: Qwen3-32B 4/8-bit, Qwen3.6-27B, Llama-3.3-70B-4bit, Gemma-4 MoE, the EAGLE-3 speculator, dspark checkpoints.
- **Verified build/API gotchas (from Task 1–3 execution) — see `docs/api/mlx-swift-api-notes.md`:** CLI must use `@main struct … static func main() async` (NOT `DispatchSemaphore`, which silently deadlocks in Swift 6; the file cannot be named `main.swift`); **`.item(_:)` internally calls `eval()`** (blocking) → the no-sync-readback loop must `asyncEval` the *next* forward before `.item()` on the current token; load API is `loadModel(from:using:)` + `#huggingFaceTokenizerLoader()` (import `MLXHuggingFace`), not `LLMModelFactory`; tokenizer is `encode(text:addSpecialTokens:)` / `decode(tokenIds:skipSpecialTokens:)`; build needs `xcodebuild … -skipPackagePluginValidation` after a one-time `xcodebuild -downloadComponent MetalToolchain`.
