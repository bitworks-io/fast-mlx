...
| As a dense-Qwen operator, I can launch the explicit OpenAI-compatible continuous no-spec route and disconnect real clients without corrupting survivors or leaking KV. | HTTP/SSE is bounded and fail-closed; real prefill/decode closes cancel within five seconds; fixed-capacity cohorts share only internally; survivor/replacement bytes remain exact; shutdown releases slots, KV, and connections. | Clean `d9143ce`: 675 XCTest + 17 Swift Testing off-box; 140 `FastMLXHarnessTests`, 176 `SpikeCoreTests`, 30 `SpikeServingAdaptersTests`, and Release through Xcode. | Fresh loaded v6: exactly 1/1 Qwen test passed in 48s; authenticated artifact and xcresult manifests; max XCTest RSS 18,231,568 KiB; no lock/watchdog/orphan. | 2026-07-24 | Explicit `continuous-batch-no-spec` only. Solo-PLD dynamic routing, broad-family default, resident transport soak, merge, and release remain open. |
