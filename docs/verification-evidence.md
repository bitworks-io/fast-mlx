...
| As an operator, I can qualify a second model family against an exact source-locked checkpoint and tokenizer rather than a mutable cache alias. | Exact public repo/revision, API/tree/per-file identities, full shard contents, tokenizer, index, and Llama geometry authenticate; partial, mutated, symlinked, mismatched, interrupted, or reused output fails closed. | Clean `dcfbbe3`; 11 authenticator tests; focused review and staged gitleaks clean. | One-shot Llama source lock: 15 files/eight shards, receipt `14512754…`, completion `f2e9082a…`, checkpoint `5083c6af…`, tokenizer `da67fb22…`; independent API/tree/sidecar validation PASS. | 2026-07-23 | Source admission only. Loaded 8K/32K/near-128K speed, capacity, loss, task, and thermal evidence remains open; Qwen KVTuner is not reusable. |
...
near-128K remains a checkpoint-limit refusal, and KVarN speed remains shelved. The next gate is a
fresh loaded-runtime qualification of the admitted, source-locked Llama-3.3-70B snapshot at
8K/32K/near-128K, without the Qwen-specific KVTuner schedule.
